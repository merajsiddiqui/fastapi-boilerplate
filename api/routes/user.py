...
from api.middlewares.auth import JWTBearer, has_subscription
...
async def otp_mobile_number(mobile_details: UserMobileVerificationRequest, Authorize: AuthJWT = Depends()):
    user = get_user_by_id(Authorize.get_jwt_subject())
    if user.mobile_number != mobile_details.mobile_number:
        return ApiResponse(messgae = "Sorry Invalid number", success = False)
...
async def verify_mobile_number(otp_details: VerifyUserMobileNumber, Authorize: AuthJWT = Depends()):
    user = get_user_by_id(Authorize.get_jwt_subject())
    if user.mobile_number != otp_details.mobile_number:
        return ApiResponse(messgae = "Sorry Invalid number", success = False)
